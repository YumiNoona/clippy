...
function renderList(){
  ...
  const libraryTotal=document.getElementById('libraryTotal');
  if(libraryTotal)libraryTotal.textContent=items.filter(x=>!x.deleted).length+' clips';
  scroll.innerHTML='';
...
const renameOverlay=document.getElementById('deviceRenameOverlay');let renameDeviceId=null;
function openRenameDevice(id){const d=devices.find(x=>x.id===id);if(!d)return;renameDeviceId=id;document.getElementById('renameDeviceInput').value=d.name;renameOverlay.classList.add('show');document.getElementById('renameDeviceInput').focus();document.getElementById('renameDeviceInput').select();}
document.getElementById('cancelRenameDevice').onclick=()=>renameOverlay.classList.remove('show');
renameOverlay.onclick=e=>{if(e.target===renameOverlay)renameOverlay.classList.remove('show');};
document.getElementById('renameDeviceForm').onsubmit=async e=>{e.preventDefault();const name=document.getElementById('renameDeviceInput').value.trim();const d=devices.find(x=>x.id===renameDeviceId);if(!d||!name)return;const submit=e.currentTarget.querySelector('[type="submit"]');submit.disabled=true;try{if(accessKey){const response=await fetch('/api/rename-device',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+accessKey},body:JSON.stringify({id:d.id,name}),signal:AbortSignal.timeout(15000)});const result=await response.json();if(!response.ok)throw new Error(result.error);devices=result.devices;}else d.name=name;await saveLocal();render();renameOverlay.classList.remove('show');toast('Device renamed.');}catch(error){toast(error.message||'Could not rename device.','offline');}finally{submit.disabled=false;}};
